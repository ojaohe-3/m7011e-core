...
public interface OrderApi {
  ...
  @Operation(
          ...
          description = "Updates the status of an existing order. Uses optimistic locking with version check."
  )
  @ApiResponses({
          @ApiResponse(
                  responseCode = "200",
  ...
  ResponseEntity<Order> updateOrderStatus(
          ...
          @RequestHeader(name = "If-Match") Long version,
          @Parameter(description = "Order status update details", required = true)
  ...
  ResponseEntity<Page<Order>> listOrders(
          @Parameter(description = "Order listing parameters", required = true)
          @ModelAttribute OrderListCommand command
  );
